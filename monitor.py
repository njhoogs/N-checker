...
COORDS_MATCH_TOLERANCE_METERS = 50


def distance_meters(lat1, lon1, lat2, lon2):
    """
    Haversine formula — actual great-circle distance in metres, matching
    the same calculation used in Apps Script's delete logic. A flat
    degree-difference check isn't equally "wide" at every latitude (a
    degree of longitude shrinks the further you get from the equator),
    so this is used instead for both dedupe and matching consistency.
    """
...
SOIL_TEMP_THRESHOLD_C = 7.0      # growth-start threshold — daily mean must hold at/above this
CONSECUTIVE_DAYS_REQUIRED = 5    # must hold at/above threshold for this many consecutive days
# Hard earliest date — the qualifying run can't START before this (month, day).
# The 7C threshold + 15-July floor together reject early warm spells that don't
# sustain. Validated against real FAR probe data at Ashburton and Methven
# (2021-2025), landing within a few days of the genuine spring lift each year.
GROWTH_CHECK_START_MONTH = 7    # July
GROWTH_CHECK_START_DAY = 15     # 15th
...
def post_with_retry(url, json=None, timeout=15, retries=2, backoff_seconds=5):
    """
    Same idea as get_with_retry, but for POST requests (e.g. Apps Script).
    Retries on connection/timeout errors AND on a 404, since a 404 right
    after redeploying Apps Script can be a brief propagation delay rather
    than a genuinely wrong URL — a short wait and retry is cheap insurance
    against that specific false alarm.
    """
    last_exception = None
    for attempt in range(retries + 1):
        try:
...
def get_soil_temp_status(lat, lon):
    ...
    # Growth-start check: daily mean has held >=7C for the required consecutive
    # days, with the qualifying run STARTING on/after the 15-July floor. The
    # higher threshold + floor reject early warm spells that don't sustain.
    from datetime import date as _date
    consecutive = 0
    run_start_day = None
    for idx in range(len(temps) - 1, -1, -1):
        if temps[idx] >= SOIL_TEMP_THRESHOLD_C:
            consecutive += 1
            run_start_day = days[idx]  # earliest day in the current run so far
        else:
            break
    floor_ok = False
    if run_start_day is not None:
        rs = _date.fromisoformat(run_start_day)
        floor_ok = (rs.month > GROWTH_CHECK_START_MONTH) or (
            rs.month == GROWTH_CHECK_START_MONTH and rs.day >= GROWTH_CHECK_START_DAY
        )
    growth_started = consecutive >= CONSECUTIVE_DAYS_REQUIRED and floor_ok
...
def get_yr_rain_forecast(lat, lon):
    ...
    try:
        ...
        # yr.no gives an entry EVERY hour, each carrying a next_6_hours precip
        # forecast for the following 6 hours. Summing every entry's next_6_hours
        # would count each rain period six times over. Walk forward in
        # non-overlapping 6-hour steps so each period is counted exactly once.
        total_rain = 0.0
        daily_totals = {}
        if timeseries:
            start_dt = datetime.fromisoformat(timeseries[0]["time"].replace("Z", "+00:00"))
            target_dt = start_dt + timedelta(days=RAIN_FORECAST_DAYS)
            next_allowed_dt = start_dt
            for entry in timeseries:
                entry_dt = datetime.fromisoformat(entry["time"].replace("Z", "+00:00"))
                if entry_dt >= target_dt:
                    break
                if entry_dt < next_allowed_dt:
                    continue  # inside a 6-hour block we've already counted
                details = entry.get("data", {}).get("next_6_hours", {}).get("details", {})
                precip = details.get("precipitation_amount")
                if precip is None:
                    continue  # no 6h block here (e.g. near end of series)
                total_rain += precip
                day = entry["time"][:10]
                daily_totals[day] = daily_totals.get(day, 0.0) + precip
                next_allowed_dt = entry_dt + timedelta(hours=6)
...
def delete_subscriber(email, name, lat, lon):
    """
    Removes a subscriber's row(s) after they've been notified, matched by
    email + farm name + coordinates — not email alone, since the same
    person can subscribe for multiple different farms and we only want to
    remove the one that actually triggered. Also not by row number, since
    row numbers go stale the moment another row is deleted earlier in the
    same run (everything below shifts up).
    """
    resp = post_with_retry(
        APPS_SCRIPT_URL,
        json={"action": "delete", "secret": APPS_SCRIPT_SECRET, "email": email, "name": name, "lat": lat, "lon": lon},
        timeout=15,
    )
    resp.raise_for_status()
    result = resp.json()
    ...
    if result.get("deleted") == 0:
        print(
            f"WARNING: delete matched 0 rows for {email} ({name}). "
            f"Searched for: {result.get('debug_searched_for')}. "
            f"Rows checked: {result.get('debug_rows_checked')}",
            file=sys.stderr,
        )
...
def process_subscribers():
    ...
    # Dedupe by email+name+coordinates — if there are duplicate rows for the
    # exact same farm subscription (e.g. from testing, or someone double-
    # submitting the form), only process it once per run. Keyed on more
    # than just email, since the same person can legitimately subscribe for
    # multiple different farms — those must each be processed independently.
    # Uses actual distance (not rounded degree equality) so this stays
    # consistent with the 50m tolerance used when deleting matched rows —
    # otherwise two rows within 50m of each other but rounding to different
    # 4-decimal values could slip past as "different" and both get emailed.
    deduped = []
    for sub in subscribers:
        email_key = (sub.get("email") or "").strip().lower()
        name_key = (sub.get("name") or "").strip().lower()
        lat, lon = sub.get("lat", 0), sub.get("lon", 0)

        is_duplicate = False
        for existing in deduped:
            existing_email = (existing.get("email") or "").strip().lower()
            existing_name = (existing.get("name") or "").strip().lower()
            if email_key != existing_email or name_key != existing_name:
                continue
            if distance_meters(lat, lon, existing.get("lat", 0), existing.get("lon", 0)) <= COORDS_MATCH_TOLERANCE_METERS:
                is_duplicate = True
                break
    ...
    for sub in subscribers:
        ...
        body = (
            ...
            f"Growth started: {result['consecutive_days_above_threshold']} consecutive days \u22657\u00b0C (from 15 Jul)\n"
            ...
            f"once the 10cm daily mean has held at or above 7\u00b0C for 5 consecutive "
            f"days, and no earlier than 15 July. This rule was validated against real "
            f"FAR soil-probe data at Ashburton and Methven across 2021-2025 — the 7\u00b0C "
            f"threshold plus the mid-July floor reject early warm spells that don't "
            f"sustain, landing within a few days of the genuine spring lift each year.\n"
            f"\n"
        ...
        if sub["email"]:
            try:
                ...
                delete_subscriber(sub["email"], sub["name"], sub["lat"], sub["lon"])
