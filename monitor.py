...
def delete_subscriber(email):
    ...
    email rather than row number — row numbers go stale the moment another
    row is deleted earlier in the same run, since everything below shifts up."""
    resp = post_with_retry(
        APPS_SCRIPT_URL,
        json={"action": "delete", "secret": APPS_SCRIPT_SECRET, "email": email},
        timeout=15,
    )
    resp.raise_for_status()
    result = resp.json()
...
def process_subscribers():
    ...
    # Dedupe by email — if there are duplicate rows for the same address
    # (e.g. from testing, or someone double-submitting the form), only
    # process it once per run rather than sending multiple emails before
    # the rows get cleaned up. delete_subscriber removes ALL matching rows
    # for that email anyway, so this just avoids redundant notifications.
    seen_emails = set()
    deduped = []
