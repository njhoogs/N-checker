...
def summary_for(date_str, trace_ids):
    ...
    if isinstance(traces, dict):
        for k, t in traces.items():
            ...
            # Harvest's summary object keys the daily mean as 'avg' (not
            # 'average') - matches harvest_join.py's summary_for(), which is
            # the code proven against this API over the 2018-2026 backfill.
            v = day.get("avg")
            try:
                v = float(v)
                if v == v:
